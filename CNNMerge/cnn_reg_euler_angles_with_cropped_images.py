...
with open('../short_data/groundtruth.json') as json_file:
...
checkPoint = keras.callbacks.ModelCheckpoint('./weights_for_xz_distance_eular_angles/weights_216_{epoch:08d}.h5',
                                             save_weights_only=False, period=5000)
...
def customDataAugGenerator(X_img, X_num, y_left, y_right, batch_size):
    while True:
        ...
        datagen = ImageDataGenerator(
            ...
            rotation_range=0,  # 180,  # randomly rotate images in the range (degrees, 0 to 180)
            width_shift_range=0,  # 0.1,  # randomly shift images horizontally (fraction of total width)
            height_shift_range=0,  # 0.1,  # randomly shift images vertically (fraction of total height)
            horizontal_flip=True,  # randomly flip images
            vertical_flip=False
        )

        batches = datagen.flow(X_img[idx], y_left[idx], batch_size=batch_size, shuffle=False,save_to_dir='/Users/vaneesh_k/PycharmProjects/Albie_ML/aug_data/')
        idx0 = 0
...
res = customDataAugGenerator(finalX_img, finalX_num, finalyLeft, finalyRight, 1)
...
model.fit([finalX_img, finalX_num], [finalyLeft, finalyRight], batch_size=1, validation_split=0.1, epochs=500000,
          verbose=1,
