...
directory = '/home/sonu/sonu/Albie_Ml/regression_modeldata1'
...
finalX_num_train = X_num[:120]
finalX_num_test = X_num[120:]
yL_train = yL[:120]
yL_test = yL[120:]
yR_train = yR[:120]
yR_test = yR[120:]
...
X_img_train = X_img
finalX_num_train = finalX_num
yL_train = yL
yR_train = yR

X_num_train.shape

print(finalX_num_train.shape, finalX_num_test.shape)
...
checkPoint = keras.callbacks.ModelCheckpoint('./weights_with_euler_angles_crooped_images/weights{epoch:08d}.h5',
                                             save_weights_only=False, period=1000)
...
batch_size =2
datagenerator = customDataAugGenerator(X_img_train,finalX_num_train,yL_train,yR_train,batch_size)

valid_datagenerator = customDataAugGenerator(X_img_test,finalX_num_test,yL_test,yR_test,batch_size)
...
model.fit_generator(generator = datagenerator, validation_data=valid_datagenerator,validation_steps=True,steps_per_epoch=len(X_img_train)/batch_size,epochs=200, verbose=1,callbacks=[tensorboard_cb])
